...
- Base discovery prefers `origin/HEAD` only when both its remote-tracking target and same-named local branch exist, then an existing conventional local base, then the current branch. Any heuristic or migrated legacy discovery can yield to a later verified remote HEAD, including a same-name provenance upgrade; an existing remote-HEAD choice remains authoritative. Base metadata and an existing tracked root are reconciled in one immediate transaction: the new base becomes parentless, old direct roots move beneath it, and the obsolete inferred base row is removed. Updates remain conditioned on the exact metadata observed so concurrent linked worktrees cannot overwrite a newer decision.
