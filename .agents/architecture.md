...
- Prefers `upstream` for sync when configured, including fork clones whose clone remote has a custom name, before falling back to the discovered base remote.
...
- Restack destinations preserve their semantic identity as either a validated immutable object ID or an exact local branch with its planned head. Ordered descendants use the exact result returned by an earlier parent restack only while that ref still matches; otherwise apply requires the branch to retain its planned head, preventing malformed object metadata, hexadecimal names, or concurrent target advances from redirecting a rewrite.
- Applies restacks parent-first and persists their sync SHAs only after every planned operation succeeds.
- For merged-parent child restacks, planning uses the freshly resolved merged PR head as immutable `old_base`. This drops commits represented by a squash merge while preserving later parent commits inherited by an open child. The PR head must resolve locally and be an ancestor of the pinned child head; missing or inconsistent fresh evidence fails closed, and cached merged state alone skips the destructive restack. The destination is the PR merge commit, or a freshly advertised remote-base commit when GitHub omits it; stale local tracking state is never used as that fallback.
- Fresh merge commits from all direct children of the base are applied as one batch. After fetch, sync selects the unique candidate that contains every merged commit and is a descendant of the planned base head, then fast-forwards once; the base must still equal that planned head and must land exactly on the reviewed target, while incomparable, divergent, or concurrent targets fail without being overwritten.
