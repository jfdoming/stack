...
- Stack metadata lives in Git's common directory so all linked worktrees share one database. A lone legacy per-worktree database is installed through an atomic no-clobber claim; concurrent or conflicting losers are left untouched and reported for manual reconciliation. Schema upgrades and parent-link cycle validation/writes are serialised transactionally across concurrent worktrees.
