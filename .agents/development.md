...
- Each restack is pinned to the child head observed during planning, and the rewritten ref is installed only if it still equals that head. If fetch, confirmation delay, or another process changes the branch before finalization, sync preserves the newer ref and asks for a fresh plan. Fallback rebases use an encoded temporary branch plus a private one-time pending ref so conflicts remain recoverable with `git rebase --continue`; the next non-dry-run sync authenticates and atomically consumes that record while finalizing the unchanged target before planning again.
- Parent restacks run before descendants, and sync SHAs are persisted only after the complete plan succeeds.
- For child restacks after a merged parent PR (including squash merges), sync anchors replay/rebase `old-base` to the freshly resolved PR head at merge time. Commits inherited from the parent after that point remain in the child; sync fails closed when the fresh head is absent, unresolvable, or outside the pinned child's history, and skips descendant rewrites based only on cached merged state. The new base must be the PR merge commit or, when that is absent, the freshly advertised remote-base commit rather than a stale local tracking ref.
- In fork workflows, `stack sync` fetches `upstream` when present so merged-parent commit SHAs can be resolved locally before replay/rebase. This preference also applies when the clone/fork remote has a custom name.
...
- When every tracked non-base branch in the stack is merged, `stack sync` prunes leaf-first only when every present local tip exactly equals fresh merged-PR `headRefOid` metadata; missing local refs can still have metadata pruned from previously OID-bound cache state.
...
- Stack metadata lives in Git's common directory so all linked worktrees share one database. A lone legacy per-worktree database is installed through an atomic no-clobber claim; concurrent or conflicting losers are left untouched and reported for manual reconciliation. Schema upgrades and parent-link cycle validation/writes are serialised transactionally across concurrent worktrees. Schema v4 persists the remote that supplied authoritative base-HEAD discovery and includes it in base reconciliation compare-and-swap checks; schema-v3 remote-HEAD rows migrate to `origin`. Verified remote HEAD discovery supersedes cached conventional, provisional, or migrated legacy base provenance while preserving an existing authoritative remote-HEAD choice; any existing tracked root is migrated atomically with that metadata change.
- Base discovery checks a persisted authoritative remote first, then the cached or heuristic base branch's configured remote, then `origin`, then a unique HEAD from remotes explicitly configured on local branches. It accepts only an existing configured remote whose direct HEAD target has a same-named local branch and a direct commit-resolvable tracking ref; dangling or chained targets are ignored and multiple broad candidates fail as ambiguous. Without authoritative evidence, it prefers an existing `main`, `master`, `trunk`, or `develop` ref and otherwise records the current-branch fallback as provisional. A later verified remote HEAD replaces conventional, provisional, or migrated legacy discovery and atomically reconciles the remote and any existing tracked root, while an established remote-HEAD choice remains stable. Missing heuristic cached refs can be repaired on startup; a missing authoritative remote or base fails unless that same remote supplies a fresh validated replacement.
